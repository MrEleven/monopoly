...
def run_backtest(strategy, file_path, symbol, start_date, end_date, config_list=[{}], log_detail=True):
    ...
    for config in config_list:
        ...
        # 提取核心精确指标
        avg_duration = trades.len.average if 'len' in trades else 0
...
def batch_backtest(strategy, data_dir="stock_data_5y", target_start="20250101", end_date="20260401", config_list=[{}], save_result=False):
    """批量处理文件夹下所有股票"""
    ...
    for j, filename in enumerate(files):
        ...
        try:
            symbol_result_list = run_backtest(strategy, file_path, symbol, target_start, end_date, config_list, log_detail=False)
            ...
            for i in range(len(config_list)):
                ...
                if result is not None:
                    ...
                    global_durations.append(result.avg_duration)
    ...
    for i in range(len(config_list)):
        ...
        print('====================================================================================')
        if save_result:
            file_name = [str.strategy_name, start_date, end_date]
            condtions = [k for k, v in cofnig.items() if (k.startswith("bc_") or k.startswith("sc_")) and v]
...
def backtest_all_config(strategy, config_list=[{}], start_date="20250101", end_date="20260417"):
    result_list = batch_backtest(strategy=ZhuAnStrategy, data_dir="stock_data_5y", target_start=start_date, end_date=end_date, config_list=config_list)
    for i in range(len(config_list)):
        config = config_list[i]
    ...
    # df_results.sort_values(by="平均单笔交易收益率(%)", ascending=False)
    # file_name = "_".join([strategy.strategy_name, start_date, end_date])
    # df_results.to_csv("result\\" + file_name + ".csv", index=False, encoding='utf_8_sig')

if __name__ == '__main__':
    backtest_all_config(ZhuAnStrategy, start_date="20250101", end_date="20260417", config_list=ZHUAN_BC_CONFIG_LIST)
    # start("002766", start_date="20220101", end_date="20260401")
    # 请确保 data_dir 指向你下载 CSV 的文件夹
    # for config in ZHUAN_BC_CONFIG_LIST:
    #     batch_backtest(strategy=ZhuAnStrategy, data_dir="stock_data_5y", target_start="20250101", end_date="20260417", config=config)
    ...
    # batch_backtest(strategy=ZhuAnStrategy, data_dir="stock_data_5y", target_start="20220101", end_date="20250101", config=config)
    # config={"log_open": True, "bc_raise_active_cap": True, 'bc_no_upper_shadow': True, 'bc_no_lower_shadow': True}
    # run_backtest(ZhuAnStrategy, "stock_data_5y/000099.csv", "000099", start_date="20250101", end_date="20260417", config=config)
